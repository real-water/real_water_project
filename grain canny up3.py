...
def emphasize_ferrite_edges(ferrite):
    # 이미지 대비를 높임
    contrast_ferrite = cv2.convertScaleAbs(ferrite, alpha=1.5, beta=0)

    # Gaussian blur를 적용하여 이미지의 노이즈를 줄임
    blurred_ferrite = cv2.GaussianBlur(contrast_ferrite, (3, 3), 0)

    # Canny 에지 검출 알고리즘의 임계값을 더 낮춤
    edges = cv2.Canny(blurred_ferrite, 20, 60)

    # 커널을 사용하여 경계선을 굵게 만듦 (크기와 반복 횟수를 더 조절)
...
cv2.imwrite("ferrite_emphasized_edges_new3.jpg", emphasized_ferrite_edges)