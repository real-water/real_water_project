...
def emphasize_ferrite_edges(ferrite):
    # Canny 에지 검출 알고리즘을 사용하여 경계를 찾음
    edges = cv2.Canny(ferrite, 0, 500)

    # 원본 페라이트 이미지와 에지를 합침
    emphasized_edges = cv2.addWeighted(ferrite, 0.8, edges, 0.2, 0)
...
cv2.imwrite("ferrite_emphasized_edges5.jpg", emphasized_ferrite_edges)
